...
- (void)viewDidLoad {
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.1f
                                     target:self selector:@selector(calculateDoppler:) userInfo:nil repeats:YES];
...
-(void) calibrate {
    ...
    NSLog(@"WTF = %f", arrayData[1000]);

    self.baselineLeftAverage = [self calcSideAverage: fftMagnitude
                                             isRight: (NO)];
    self.baselineRightAverage = [self calcSideAverage:fftMagnitude
                                              isRight: (YES)];
