...
public func product <S :Sequence> (s: S) -> Double where S.Iterator.Element == Double {
    return s.reduce(Double(1.0), *)
}

public func sum <S :Sequence> (s: S) -> Double where S.Iterator.Element == Double {
    return s.reduce(Double(0.0), +)
}

public func argmax <T, V: Comparable> (collection :[T:V]) -> T? {
    return vararg(f: { $0 > $1 })(collection)
}
...
private func vararg <T, V: Comparable> (f: @escaping (V, V) -> Bool) -> (_ collection :[T:V]) -> T? {
